/**
 * Country Diversity — Full recompute (requires author enrichment)
 * Formula: min(uniqueCountries / 10, 1) × 100 × evennessBonus
 * evennessBonus = 1 - (avgLogsPerCountry / maxLogsPerCountry × 0.3)
 */

export function computeCountryDiversity(logs, authorCountryMap) {
  // authorCountryMap: { authorKey: countryCode }
  ...
  // Quality gate: >50% unresolvable → suppress
  const totalAuthors = resolvableCount + unresolvableCount;
  if (totalAuthors === 0) return null;
  if (unresolvableCount / totalAuthors > 0.5) return null;
