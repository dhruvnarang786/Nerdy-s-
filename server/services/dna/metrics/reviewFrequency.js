/**
 * Review Frequency — Incremental
 * Formula: (logsWithNonEmptyNotes / totalLogs) × 100
 */
...
export function applyReviewFrequencyDelta(accum, log, isCreate) {
  ...
  if (isCreate) {
    a.totalLogs++;
    if (hasNotes) a.logsWithNotes++;
  } else {
    a.totalLogs = Math.max(0, a.totalLogs - 1);
    if (hasNotes) a.logsWithNotes = Math.max(0, a.logsWithNotes - 1);
  }
  return a;
}
