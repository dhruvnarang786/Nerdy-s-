...
export class DnaQueryService {
  /**
   * Fetch a user's DNA snapshot with staleness check.
   */
  async getSnapshot(userId, fields = []) {
    ...
    // Map field groups to snapshot JSON fields
    const fieldMap = {
    ...
    if (fields.length === 0 || fields.includes('all')) {
      for (const group of Object.keys(fieldMap)) {
        for (const f of fieldMap[group]) resolvedFields.add(f);
      }
    } else {
      for (const field of fields) {
        const mapped = fieldMap[field];
        if (mapped) for (const f of mapped) resolvedFields.add(f);
      }
    }
    ...
    // Badges: fetch from Badge table if requested
    ...
    // Genres: fetch from GenreAffinity table if requested
  ...
  async compare(userId, friendUserId) {
    ...
    if (!userSnapshot || !friendSnapshot) return null;
